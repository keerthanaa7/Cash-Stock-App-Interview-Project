...
class MainActivity : ComponentActivity() {
    ...
    fun stockAppUI() {
        ...
        var stockResponseState = stockViewModel.stockList.observeAsState()
        var stockList: List<Stock>? = stockResponseState.value

        LazyColumn {
            stockList?.size?.let {
                items(it) { index ->
                    Log.d(TAG, " stock name " + stockList[index].name + "price " + stockList[index].current_price_cents + " currency " + stockList[index].currency+
                    " ticker " + stockList[index].ticker + " quanty " + stockList[index].quantity)
                    Row(
                        modifier = Modifier
                            .fillMaxWidth()
                            .padding(8.dp),
